...
class StretchManipulation:
    def __init__(self):
        ...
        self.client = zmq_client.SocketClient(
            zmq_client.IP_DESKTOP, port=zmq_client.PORT_COMMAND_SERVER)

        self.server = zmq_server.SocketServer(
            port=zmq_client.PORT_STATUS_SERVER)
        self.thread = zmq_client.DaemonStoppableThread(
            0.02, target=self.publish_status_loop, name='status_sender')
